        ...
        <table class="table table-bordered table-hover table-sm mb-0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>{{ $value->Rapper->name }}</td>
                    <td>{{ $value->Beat->beatname }}</td>
                    <td>
                        <span class="d-block fw-bold text-primary"><a>{{ $value->BaiViet->tieude }}</a></span>
                        <span class="d-block small">
                            Ngày đăng:<strong>{{ Carbon\Carbon::createFromFormat('Y-m-d H:i:s',
                                $value->created_at)->format('d/m/Y H:i:s') }}</strong>
                            <br />Nội dung bình luận: <strong>{{ $value->noidungbinhluan }}</strong></span>
                    </td>
